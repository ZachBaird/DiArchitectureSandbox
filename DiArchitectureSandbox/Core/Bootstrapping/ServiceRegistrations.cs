...
namespace DiArchitectureSandbox.Core.Bootstrapping;

// Our interfaces for service registration.
    ...
    public static void RegisterServices(this IServiceCollection services)
    {
        var result = (from type in Assembly.GetExecutingAssembly().GetTypes()
                      where (type.GetInterfaces().Contains(typeof(IDiSingleton))
                          ...
                          || type.GetInterfaces().Contains(typeof(IDiTransient))) && type.GetConstructor(Type.EmptyTypes) != null
                      select type).ToList();
